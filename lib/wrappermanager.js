...
(function () {
    ...
    WrapperManager.prototype._fetchImports = function() {
        ...
        this._componentManager.getAllComponentTypes().forEach(function(componentType){
            // maybe flag for button import
            if(componentType == "Button") hasButtonHandler = true;

            // get class path for current component type
            imports.push(this._getClassPathForComponentType(componentType));
        }, this);

        // maybe add button handler event import
        if(hasButtonHandler) imports.push("starling.events.Event");

        // todo: determine base class (UiElement or BasePopup) via document name
        imports.push("com.mobscience.match.ui.popup.BasePopup");

        // todo: don't *always* import these... only when absolutely necessary!
        imports.push("starling.display.DisplayObject");
        imports.push("starling.display.Sprite");

        // return alphabetically sorted import strings
        return imports.sort().map(function(classPath){
    ...
    WrapperManager.prototype._fetchMembers = function() {
        ...
        this._componentManager.getAllComponents().forEach(function(component){
            members.push(`${component.name}:${this._getClassTypeForComponentType(component.type)}`);
        }, this);
    ...
    WrapperManager.prototype._fetchInitializers = function() {
        // return all member instantiation strings
        return this._componentManager.getAllComponents().map(function(component) {
            return `            _${component.name} = LayoutUtils.initAsset("${component.name}", layout, this) as ${this._getClassTypeForComponentType(component.type)};`;
        }, this).join(`\n`);
    };
    ...
    WrapperManager.prototype._getClassPathForComponentType = function(componentType) {
        switch(componentType) {
            case "Image":
                return "starling.display.Image";
            case "Text":
    ...
    WrapperManager.prototype._getClassTypeForComponentType = function(componentType) {
        switch(componentType) {
            case "Image":
                return "Image";
            case "Text":
