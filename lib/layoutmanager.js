...
(function () {
    ...
    /**
     * The Layout Manager renders layout XML for 
    ...
    /**
     * Render Layout XML using each component inside document
     * 
     * @private
     */
    LayoutManager.prototype.render = function() {
        var layergroup = {
            layers: this._componentManager.getAllComponentLayers()
        };
        var layout = 
            '<?xml version="1.0" encoding="UTF-8"?>' + "\n" +
            `<Layout width="${this._document.bounds.right * 0.5}" height="${this._document.bounds.bottom * 0.5}">` + "\n" +
            this._fetchSubObjects(layergroup, 1) +
            '</Layout>';
